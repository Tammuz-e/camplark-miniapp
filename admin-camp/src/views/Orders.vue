    ...
    <div class="page-card">
      ...
      <a-table-column title="操作" :width="200" :fixed="'right'">
        <template #default="{ record }">
          ...
          <a-button v-if="record.status === '已入营'" size="small" @click="checkout(record)">办理退房</a-button>
          <a-button v-if="record.status === '待入营'" size="small" status="danger" @click="cancel(record)">取消订单</a-button>
        </template>
      </a-table-column>
      </a-table>
