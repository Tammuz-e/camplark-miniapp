<template>
  <div class="page-container">
    <div class="page-card">
      ...
      <a-button v-if="canManage" type="primary" @click="showAdd">+ 新增营位</a-button>
    </div>
      ...
      <a-form :model="form" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }">
        <a-form-item label="所属营地">
          <a-select v-model="form.campId" placeholder="选择营地" style="width:100%" :disabled="!!myCampId">
        ...
        <a-form-item label="价格"><a-input v-model="form.price" placeholder="如：¥900/天" /></a-form-item>
...
import { getCamps } from '../api/campList'
import { getSpots, createSpot, updateSpot, setSpotStatus } from '../api/spot'
...
const myCampId = info.campId
const canManage = !!myCampId
...
async function loadCamps() {
  try {
    ...
    let campList = data.data || []
    if (myCampId) campList = campList.filter(c => c.id === myCampId)
...
async function load() {
  loading.value = true
  try {
    const { data } = await getSpots({ status: filterStatus.value })
...
function showAdd() {
  formId.value = null
  form.campId = myCampId || camps.value[0]?.id || null
  form.spotName = ''
  form.price = ''
  form.priceNum = 0
...
async function save() {
  if (!form.campId || !form.spotName) {
    Message.warning('请填写营位名称和选择营地')
    return
  }
  try {
    if (formId.value) {
